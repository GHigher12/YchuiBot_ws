...
boost::asio::serial_port sp(iosev, "/dev/Ychuibot");
...
const double ROBOT_LENGTH = 427.00;  //mm
const double ROBOT_RADIUS = 85.00;  //两轮之间的半径长度mm
...
void writeSpeed(double RobotV, double YawRate,unsigned char ctrlFlag)
{
    ...
    if(RobotV == 0)      //旋转
    {
        ...
        leftVelSet.d  = (short)(-YawRate * 60 / 2 / PI);//mm/s
        rightVelSet.d = (short)(YawRate * 60 / 2 / PI);//mm/s
    } 
    else if(YawRate == 0)//直线
    {
        // leftVelSet.d  = (short)RobotV;//mm/s
        // rightVelSet.d = (short)RobotV;
        leftVelSet.d  = (short)(RobotV * 60 / ROBOT_RADIUS / 2 / PI);//mm/s
        rightVelSet.d = (short)(RobotV * 60 / ROBOT_RADIUS / 2 / PI);
    }
    else                //速度不一致
    {
        // leftVelSet.d  = (short)(YawRate * (r - ROBOT_RADIUS));//mm/s
        // rightVelSet.d = (short)(YawRate * (r + ROBOT_RADIUS));
        leftVelSet.d  = (short)((YawRate * (r - ROBOT_LENGTH / 2))* 60 / ROBOT_RADIUS / 2 / PI);//mm/s
        rightVelSet.d = (short)((YawRate * (r + ROBOT_LENGTH / 2))* 60 / ROBOT_RADIUS / 2 / PI);
    }
...
bool readSpeed(double &vx,double &vth,double &th,unsigned char &ctrlFlag)
{
    ...
    // 打印数据信息
    // 线速度(mm/s) = 转速 * 轮子周长 / 60 (mm/s)
    leftVelNow.d = (short)(leftVelNow.d * 2 * PI * ROBOT_RADIUS / 60);
    rightVelNow.d = (short)(rightVelNow.d * 2 * PI * ROBOT_RADIUS / 60);
    ROS_INFO("Left_v:%d\n",leftVelNow.d);
    ROS_INFO("Right_v:%d\n",rightVelNow.d);
    ROS_INFO("Angle:%d\n",angleNow.d);
    // ROS_INFO("crtlFlag:%d\n",ctrlFlag);
